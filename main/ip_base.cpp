...
#ifdef UDPDEBUG_HS
	{
		printf(MSGHDR"sending handshake %i (t%i state %i(%s)) for (%p)",hsb->attempts,hsb->type,hsb->state,ip_hs_statetoa(hsb->state),addr.goodaddr);
		ip_addr_list::iterator i;
		for (i=addr.begin();i!=addr.end();i++) {
			if (i!=addr.begin())
				printf(", ");
...
#ifdef UDPDEBUG_HS
					printf(MSGHDR"handshake timeout (state %i(%s)) for (%p)",hsb->state,ip_hs_statetoa(hsb->state),peer->addr.goodaddr);
					ip_addr_list::iterator i;
					for (i=peer->addr.begin();i!=peer->addr.end();i++) {
						if (i!=peer->addr.begin())
							printf(", ");
						i->dump();
					}
					printf(" v%i\n",peer->iver);
