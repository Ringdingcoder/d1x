...
/*		gr_printf(0,47+j*7,
			"%8s: %3i/%3i(%3i%%) %3s %s",
			Players[j].callsign,
			Players[j].net_kills_total,
			Players[j].net_killed_total,
			(Players[j].net_killed_total+Players[j].net_kills_total)?(Players[j].net_kills_total*100)/(Players[j].net_killed_total+Players[j].net_kills_total):0,
			Net_D1xPlayer[j].shp?"yes":"no",
			Net_D1xPlayer[j].ver);*/
	}
}
#endif
...
//killed 10/9/98 by Victor Rachels to replace by hud_message
//-killed-      if (!Allow_weapon_cycle)
//-killed-       {
//-killed-        char message[128];
//-killed-        int h,w,aw;
//-killed-
//-killed-         sprintf(message, "Autoselect OFF");
//-killed-         gr_set_curfont( GAME_FONT );
//-killed-         gr_set_fontcolor(gr_getcolor(0,27,0), -1 );
//-killed-         gr_get_string_size(message, &w,&h,&aw);
//-killed-          if (Cockpit_mode == CM_FULL_COCKPIT)
//-killed-           h += 15;
//-killed-          else if ( Cockpit_mode == CM_LETTERBOX )
//-killed-           h += 7;
//-killed-         
//-killed-          if (Cockpit_mode != CM_REAR_VIEW)
//-killed-           gr_printf(grd_curcanv->cv_bitmap.bm_w-w-5, 5, message );
//-killed-       }
//end kill
//end addition
...
//    newmenu_do( NULL, TXT_KEYS, 6, m, NULL );
...
//			if (!(Game_mode&GM_MULTI)) {//bad texture could arguably possibly help in multi, if the bad texture was transparent
