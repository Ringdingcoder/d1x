...
//end this section addition - VR
	...
	opt = 0;
        /* 0*/ m[opt].type = NM_TYPE_CHECK; m[opt].text = "Short packets"; /*m[opt].value = (flags & NETFLAG_SHORTPACKETS) != 0;*/ opt++;
	/* 1*/ m[opt].type = NM_TYPE_TEXT; m[opt].text = "Packets per second (2 to 20)"; opt++;
        /* 2*/ m[opt].type = NM_TYPE_INPUT; /*m[opt].text = spps;*/ m[opt].text_len = 2; opt++;
	/* 3*/ m[opt].type = NM_TYPE_TEXT; m[opt].text = "Network socket (-99 to 99)"; opt++;
        /* 4*/ m[opt].type = NM_TYPE_INPUT; /*m[opt].text = ssocket;*/ m[opt].text_len = 3; opt++;

	/* 5*/ m[opt].type = NM_TYPE_TEXT; m[opt].text = "Options below need D1X only games"; opt++;
        /* 6*/ m[opt].type = NM_TYPE_CHECK; m[opt].text = "D1X only game"; /*m[opt].value = (temp_game->protocol_version == MULTI_PROTO_D1X_VER);*/ opt++;
//added/edited on 11/1/98 by Matthew Mueller
        /* 7*/ m[opt].type = NM_TYPE_SLIDER; m[opt].text = smaxplayers; /*m[opt].value = temp_game->max_numplayers - 2;*/ m[opt].min_value = 0; m[opt].max_value = MaxNumNetPlayers - 2; opt++;
//end edit -MM
	/* 8*/ m[opt].type = NM_TYPE_MENU; m[opt].text = "Set objects allowed..."; opt++;
        /* 9*/ m[opt].type = NM_TYPE_CHECK; m[opt].text = "Drop vulcan ammo"; /*m[opt].value = (flags & NETFLAG_DROP_VULCAN_AMMO) != 0;*/ opt++;
        /*10*/ m[opt].type = NM_TYPE_CHECK; m[opt].text = "Enable ignore/ghost"; /*m[opt].value = (flags & NETFLAG_ENABLE_IGNORE_GHOST) != 0;*/ opt++;
